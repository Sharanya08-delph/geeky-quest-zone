...
const MemberLeaderboard = () => {
  ...
  const getBadge = (solved: number) => {
    if (solved >= 300) return { label: "Diamond", color: "bg-blue-500/10 text-blue-400 border-blue-500/20" };
    if (solved >= 200) return { label: "Platinum", color: "bg-purple-500/10 text-purple-400 border-purple-500/20" };
    if (solved >= 100) return { label: "Gold", color: "bg-yellow-500/10 text-yellow-400 border-yellow-500/20" };
    if (solved >= 50) return { label: "Silver", color: "bg-gray-400/10 text-gray-300 border-gray-400/20" };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {sorted.slice(0, 3).map((user, i) => (
          ...
          >
            ...
            <p className="text-xs text-muted-foreground">{user.department} • {user.year} Year</p>
            <p className="font-mono text-2xl font-bold text-primary mt-2">{user.problemsSolved}</p>
            <p className="text-xs text-muted-foreground">problems solved</p>
            <div className={`inline-block mt-3 px-3 py-1 rounded-full text-xs font-bold border ${getBadge(user.problemsSolved).color}`}>
              {getBadge(user.problemsSolved).label}
            </div>
          </motion.div>
        ))}
      </div>
      ...
      <div className="glass-card overflow-hidden">
        <table className="w-full">
          ...
          <tbody>
            {sorted.map((u, i) => {
              const badge = getBadge(u.problemsSolved);
              return (
                <tr key={u.id} className="border-b border-border/50 hover:bg-accent/50 transition-colors cursor-pointer">
                  <td className="px-4 py-3 font-mono font-bold text-muted-foreground">#{i + 1}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-primary/20 flex items-center justify-center text-primary text-sm font-bold">{u.name.charAt(0)}</div>
                      <span className="font-medium text-foreground">{u.name}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-sm text-muted-foreground">{u.department}</td>
                  <td className="px-4 py-3 font-mono font-bold text-foreground">{u.problemsSolved}</td>
                  <td className="px-4 py-3 font-mono text-streak-orange">{u.streak}🔥</td>
                  <td className="px-4 py-3">
