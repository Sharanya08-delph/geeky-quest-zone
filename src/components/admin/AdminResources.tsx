...
const AdminResources = () => {
  ...
  const [form, setForm] = useState({ title: "", category: "tutorial", url: "", description: "", type: "tutorial" });

  const handleSubmit = async () => {
    if (!form.title || !form.url) return;
    await addResource({ ...form });
    setForm({ title: "", category: "tutorial", url: "", description: "", type: "tutorial" });
    setShowForm(false);
  };
    ...
    <div className="space-y-6">
      ...
      {showForm && (
        <div className="glass-card p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <select value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value, type: e.target.value }))} className="input-field">
      ...
      <div className="glass-card overflow-hidden">
        <table className="w-full">
          ...
          <tbody>
            {resources.length === 0 ? (
              <tr><td colSpan={4} className="px-4 py-6 text-center text-muted-foreground">No resources yet.</td></tr>
            ) : (
              resources.map(r => (
                <tr key={r.id} className="border-b border-border/50 hover:bg-accent/50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-sm text-muted-foreground capitalize">{r.category || r.type}</td>
                  <td className="px-4 py-3 text-sm font-mono text-muted-foreground">{new Date(r.created_at).toLocaleDateString()}</td>
